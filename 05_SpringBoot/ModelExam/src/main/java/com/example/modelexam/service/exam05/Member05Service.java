...
//        사원번호가 없으면 insert문 실행
        if(member.getEno() == null) {
//            1) 사원 번호 자동생성 : 8000+count (현재 사원정보으 ㅣ총 건수)
            int count = memberDao.selectAll().size(); // 건수
            int newEno = (count + 8000); // 시로운 사원번호(8004)
            member.setEno(newEno); // 새로운 사원번호를 Member 객체의 eno속성에 저장 
...
//            LocalDateTime.now().toString() : 현재 시간 구하기
            member.setInsertTime(LocalDateTime.now().format(dtf)); // 오늘날짜 저장

            list = memberDao.insert(member); // db반영
        }
