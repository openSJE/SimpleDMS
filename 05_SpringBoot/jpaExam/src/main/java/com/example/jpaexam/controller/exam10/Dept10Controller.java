...
public class Dept09Controller {
    ...
    @Autowired
    Dept09Service dept09Service; // @Autowired : Springboot 가 가동될 때 생성된 객체를 하나 받아오기
    ...
    public ResponseEntity<Object> getDeptAll() {
        ...
        try {
            List<Dept> list = dept09Service.findAll();
...
//            서버에서 발생 메세지 전송 (클라이언트)
    ...
    public ResponseEntity<Object> getDeptId(@PathVariable int dno) {
        ...
        try {
            Optional<Dept> ooptionalDelpt = dept09Service.findById(dno);
...
//    @RequestBody : 클라이언트에서 json 객체를 전송하면 스프링부트 함수에서 매개변수로 전달받음
    ...
    public ResponseEntity<Object> createDept(@RequestBody Dept dept) {
        ...
        try {
            Dept dept2 = dept09Service.save(dept); // insert 문 호출
