...
public class Dept11Controller {
    ...
    @Autowired
    Dept11Service dept11Service; // @Autowired : Springboot 가 가동될 때 생성된 객체를 하나 받아오기
    ...
    public ResponseEntity<Object> getDeptAll() {
        ...
        try {
            List<Dept> list = dept11Service.findAll();
...
//            서버에서 발생 메세지 전송 (클라이언트)
    ...
    public ResponseEntity<Object> getDeptId(@PathVariable int dno) {
        ...
        try {
            Optional<Dept> ooptionalDelpt = dept11Service.findById(dno);
...
//    @RequestBody : 클라이언트에서 json 객체를 전송하면 스프링부트 함수에서 매개변수로 전달받음
    ...
    public ResponseEntity<Object> createDept(@RequestBody Dept dept) {
        ...
        try {
            Dept dept2 = dept11Service.save(dept); // insert 문 호출
...
//            매개변수 dept 안에 부서번호(기본키) 값이 있음 -> update 문 실행
            Dept dept2 = dept11Service.save(dept); // update 문 호출
...
//          id에 해당하는 부서정보를 삭제하는 함수 호출
        boolean bSuccess = dept11Service.removeById(dno); // Delete 문 호출
